  ...
  )
{
  ...
  Status = HeciSendwAck(HECI1_DEVICE, (UINT32*)&HeciMsg.Req,
                            sizeof(HeciMsg.Req), &RspLen,
                            SPS_CLIENTID_BIOS, SPS_CLIENTID_ME_MKHI);

  printf ("HFSTS1 = 0x%08X\n", HeciPciRead32(SPS_REG_MEFS1));
  //DEBUG((DEBUG_INFO, "[SPS] MEFS1: 0x%08X\n", HeciPciRead32(SPS_REG_MEFS1)));

  if (EFI_ERROR(Status)) {
    printf ("HMRFPO_ENABLE sending failed, Status = %llx\n",Status);
    //DEBUG((DEBUG_ERROR, "[SPS] ERROR: HMRFPO_ENABLE sending failed (%r)\n",Status));
    return Status;
  }
  ...
  if (HeciMsg.Rsp.Status != 0) {
    ...
    //DEBUG(
    //    (DEBUG_ERROR, "[SPS] ERROR: HMRFPO_ENABLE refused (cause: %d)\n", HeciMsg.Rsp.Status));
    return EFI_UNSUPPORTED;
  }
  printf ("HMRFPO_ENABLE command success\n");
  //DEBUG((DEBUG_INFO, "[SPS] HMRFPO_ENABLE reports success.\n"));
...
int main(void){
    ...
    printf ("hmrfpo_enable version : 1.6\n");
